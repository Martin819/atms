...
public class ErsteController {
    ...
    private ErsteBranch retBranch;
    private static final String BASE_URL = "https://webapi.ersteapihub.com/";
    ...
    private ErsteService ersteService = retrofit.create(ErsteService.class);
    public ErsteController(Activity activity) {
    ...
    public void getBranchesList(String apikey, BranchesAdapter branchesAdapter, boolean forceFetch) {
        ...
        if (forceFetch) {
            ...
            Call<ErsteBranchesList> branchesListCall = ersteService.getBranchesList(apikey);
            branchesListCall.enqueue(new Callback<ErsteBranchesList>() {
                @Override
                public void onResponse(@NonNull Call<ErsteBranchesList> call, @NonNull Response<ErsteBranchesList> response) {
                    ...
                    if (response.isSuccessful()) {
                        ...
                        ErsteBranchesList fetchedList = response.body();
                        if (fetchedList != null) {
                            new Thread(() -> {
                                UniBranchesList uniBranchesList = new UniBranchesList(fetchedList);
                ...
                @Override
                public void onFailure(@NonNull Call<ErsteBranchesList> call, @NonNull Throwable t) {
                    t.printStackTrace();
                }
            });
        } else {
    ...
    public void getBranch(String apikey, String branchId) {
        Log.d(DEBUG_TAG_INFO, "ErsteController.getBranch called");
        ...
        Call<ErsteBranch> branchCall = ersteService.getBranch(branchId, apikey);
        branchCall.enqueue(new Callback<ErsteBranch>() {
            @Override
            public void onResponse(@NonNull Call<ErsteBranch> call, @NonNull Response<ErsteBranch> response) {
                Log.d(DEBUG_TAG_INFO, "getBranch.onResponse called");
                Log.d(DEBUG_TAG_INFO, response.toString());
                if(response.isSuccessful()) {
                    Log.d(DEBUG_TAG_INFO, "getBranch - response.isSuccessful()");
                    ErsteBranch branch = response.body();
                    if (branch != null) {
...
/*                        String branchPictures[] = branch.getPictures();
            ...
            @Override
            public void onFailure(@NonNull Call<ErsteBranch> call, @NonNull Throwable t) {
                t.printStackTrace();
            }
        });
    }
    ...
    public void getATMsList(String apikey, ATMsAdapter atmsAdapter, boolean forceFetch) {
        ...
        if (forceFetch) {
            Log.d(DEBUG_TAG_INFO, "ErsteController.getATMsList called");
            Call<ErsteATMsList> atmsListCall = ersteService.getATMsList(apikey);
            atmsListCall.enqueue(new Callback<ErsteATMsList>() {
                @Override
                public void onResponse(@NonNull Call<ErsteATMsList> call, @NonNull Response<ErsteATMsList> response) {
                    Log.d(DEBUG_TAG_INFO, "getATMsList.onResponse called");
                    Log.d(DEBUG_TAG_INFO, response.toString());
                    if (response.isSuccessful()) {
                        Log.d(DEBUG_TAG_INFO, "getATMsList - response.isSuccessful()");
                        ErsteATMsList fetchedList = response.body();
                        if (fetchedList != null) {
                            new Thread(() -> {
                                UniATMsList uniATMsList = new UniATMsList(fetchedList);
                ...
                @Override
                public void onFailure(@NonNull Call<ErsteATMsList> call, @NonNull Throwable t) {
                    t.printStackTrace();
                }
            });
        } else {
    ...
    public void getATM(String apikey, String ATMId) {
        ...
        Call<ErsteATM> ATMCall = ersteService.getATM(ATMId, apikey);
        ATMCall.enqueue(new Callback<ErsteATM>() {
            @Override
            public void onResponse(@NonNull Call<ErsteATM> call, @NonNull Response<ErsteATM> response) {
                Log.d(DEBUG_TAG_INFO, "getATM.onResponse called");
                Log.d(DEBUG_TAG_INFO, response.toString());
                if(response.isSuccessful()) {
                    Log.d(DEBUG_TAG_INFO, "getATM - response.isSuccessful()");
                    ErsteATM atm = response.body();
                    if (atm != null) {
                        Log.d(DEBUG_TAG_INFO, "getATM - branch != null");
                        Log.d(DEBUG_TAG_INFO, "getATM - response: " + utils.getFullAddress(atm));
                        activity.setTitle(R.string.title_atm);
                        mATMAddress.setText(utils.getFullAddress(atm));
                        utils.getErstePlaceOpeningHours(atm, activity);
                        utils.getErstePlaceOpeningHours(atm, activity);
                    } else {
            ...
            @Override
            public void onFailure(@NonNull Call<ErsteATM> call, @NonNull Throwable t) {
                t.printStackTrace();
            }
        });
    }
